...
vqvae_wideresnet_128 = Hyperparams(
    sr = 22050,
    levels = 1,
    downs_t = (5,),
    strides_t = (2,),
...
HPARAMS_REGISTRY["vqvae_wideresnet_128"] = vqvae_wideresnet_128

single_vqvae= Hyperparams(
    sr = 22050,
    levels = 1,
    downs_t = (5,),
    strides_t = (2,),
...
HPARAMS_REGISTRY["single_vqvae"] = single_vqvae

single_prior_64M = Hyperparams(
    n_ctx=3584,  # length of longest audio sample in the dataset as VQVAE code length (previously 3584/7166)
    prior_width=512,
    prior_depth=72,
    heads=1,
    c_res=1,
    attn_order=10,  # This decides if attending to lyrics is used or not
    blocks=64,
    init_scale=0.1,
    beta2=0.925,  # copied from Jukebox 5b model parameters
    single_enc_dec=False,
    use_tokens=True,
    n_tokens=192,  # length in letters of longest sentence in the dataset (previously 192)
    n_vocab=79,  # number of letters and symbols in text vocab
    labels=True,
    fp16_params=False,
    restore_prior='',
    alignment_layer=18,  # used for jukebox lyrics alignment, try smaller value?
    alignment_head=0,  # used for jukebox lyrics alignment
    prime_loss_fraction=0.4,  # fraction of cross-entropy loss related to text label encoder (Transformer)
    prime_width=512,
    prime_depth=3,
    prime_heads=2,
    prime_attn_order=2,
    prime_blocks=16,  # 32 need to be multiple of n_tokens
    prime_init_scale=0.7,
    prime_c_res=1,  # merged_decoder=True,
    emb_dropout=0.0,
)
HPARAMS_REGISTRY["single_prior_70M_test"] = single_prior_70M_test

single_upsampler = Hyperparams(
    n_ctx=3584,
    prior_width=512,
    prior_depth=79,
    heads=4,
...
HPARAMS_REGISTRY["single_upsampler"] = single_upsampler
...
upsampler_level_0 = Hyperparams(
    level=0,
    restore_prior='jukebox/models/5b/prior_level_0.pth.tar'
)
...
DEFAULTS["script"] = Hyperparams(
    ...
    debug_inputs=False,
    local_path='',
    local_logdir='logs_prior',
    max_len=24,
    max_log=32,
    save=True,
    save_iters=4000,  # 20000
    seed=0,
    prior=False,
    log_steps=100,
    func='',
...
DEFAULTS["prior"] = Hyperparams(
    ...
    alignment_head=None,
    multispectral_loss_fraction=0.0,
)
...
DEFAULTS["audio"] = Hyperparams(
    ...
    use_nonrelative_specloss=False,  # True
