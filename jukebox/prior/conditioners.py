...
class RangeEmbedding(nn.Module):
    ...
    def forward(self, pos_start, pos_end=None):
        ...
        if n_time != 1:
            ...
            print(f"conditioners.py RangeEmbedding pos_start {pos_start}, pos_end {pos_end}, n_time {n_time}")
            print(f"conditioners.py RangeEmbedding interpolation {interpolation}")
...
class LabelConditioner(nn.Module):
    ...
    def forward(self, y):
        ...
        if self.include_time_signal:
            ...
            pos_emb = None
        print(f"conditioners.py LabelConditioner start_emb {start_emb.shape} {start_emb}")
        print(f"conditioners.py LabelConditioner start_emb {pos_emb.shape} {pos_emb}")
...
class LabelConditioner(nn.Module):
    ...
    def forward(self, y):
        if self.include_time_signal:
            ...
            print(f"conditioners.py LabelConditioner forward total_length {total_length}, {offset}, {length}, {artist}, {genre}")
        ...
        if self.include_time_signal:
            start, end = offset, offset + length
            total_length, start, end = total_length.float(), start.float(), end.float()
            print(f"self.total_length_emb(total_length) {self.total_length_emb(total_length)}")
            print(f"self.absolute_pos_emb(start, end) {self.absolute_pos_emb(start, end)}")
            print(f"self.relative_pos_emb(start/total_length, end/total_length) {self.relative_pos_emb(start / total_length, end / total_length)}")
            pos_emb = self.total_length_emb(total_length) + self.absolute_pos_emb(start, end) + self.relative_pos_emb(start/total_length, end/total_length)
            print(f"conditioners.py LabelConditioner pos_emb {pos_emb.shape} {pos_emb}")
            assert_shape(pos_emb, (N, self.n_time, self.out_width))
        else:
            pos_emb = None
        # print(f"conditioners.py LabelConditioner start_emb {start_emb.shape} {start_emb}")
        # return start_emb, pos_emb
        #print(f"conditioners.py LabelConditioner forward pos_emb {pos_emb}")
        #return pos_emb
        return y